...
from open_instruct.search_rewards.utils.rubric_chat_templates import DEFAULT_RUBRIC_GENERATION_SYSTEM_PROMPT
...
def rlvr_tokenize_v3(
    ...
    **_unused_kwargs: Any,
    ...
    # Keep compatibility with callers that share transform kwargs across
    # multiple tokenizers (e.g., question/accepted/rejected answer keys).
...
def rubric_judge_tokenize_v2(
    ...
    """
    Transform dataset with question, accepted_answer, rejected_answer format.
    Creates messages format and ground truth structure for rubric judge reward.
    The policy model generates a rubric given the question.
    """
    question = row[question_key]
    accepted_answer = row[accepted_answer_key]
    rejected_answer = row[rejected_answer_key]

    # Create messages format: user asks question, assistant generates rubric
    prompt = []

    # Add system prompt - use override if provided, otherwise use canonical prompt
    system_prompt = system_prompt_override if system_prompt_override else DEFAULT_RUBRIC_GENERATION_SYSTEM_PROMPT
    ...
    # Store ground truth with accepted and rejected answers for reward computation
    ground_truth = {
        "question": question,
        "accepted_answer": accepted_answer[-1]["content"],
        "rejected_answer": rejected_answer[-1]["content"],
    }
    row[GROUND_TRUTHS_KEY] = json.dumps(ground_truth)

    # Remove original keys to avoid conflicts
    row.pop(question_key, None)
    row.pop(accepted_answer_key, None)
    row.pop(rejected_answer_key, None)
    ...
    if verifier_source_key in row:
        ...
        row[VERIFIER_SOURCE_KEY] = ["rubric_judge"]

    # Store raw prompt
    row[RAW_PROMPT_KEY] = question

    return row


def dapo_math_remap(row: Dict[str, Any], tokenizer: PreTrainedTokenizer, **_unused_kwargs: Any) -> Dict[str, Any]:
    """Remap DAPO-Math-17k (open-r1/DAPO-Math-17k-Processed) columns to rlvr format.

    DAPO schema:
        source_prompt  -> list[dict]  (message dicts with role/content)
        reward_model   -> dict        (contains 'ground_truth' and 'style')
        data_source    -> str         ("math_dapo")

    rlvr expected schema:
        messages       -> list[dict]  (message dicts)
        ground_truth   -> str|list    (answer string)
        dataset        -> str|list    (verifier type, e.g. "math")
    """
    # Map source_prompt (list of message dicts) -> messages
    row["messages"] = row["source_prompt"]

    # Extract ground truth from nested reward_model dict
    reward_model = row.get("reward_model", {})
    row["ground_truth"] = reward_model.get("ground_truth", "")

    # Map verifier type: DAPO uses rule-lighteval/MATH_v2 style -> "math" verifier
    row["dataset"] = "math"

    return row


def dapo_math_rlvr_tokenize(
    row: Dict[str, Any],
    tokenizer: PreTrainedTokenizer,
    sft_messages_key: str = DEFAULT_SFT_MESSAGES_KEY,
    ground_truths_key: str = GROUND_TRUTHS_KEY,
    ...
    **_unused_kwargs: Any,
    ...
    """Combined remap + tokenize for DAPO-Math-17k datasets.

    First remaps DAPO columns to rlvr format, then tokenizes.
    This is a single-step alternative to dapo_math_remap + rlvr_tokenize_v1,
    ensuring compatibility with setup_datasets() which expects exactly 2 transforms
    (tokenize + filter).

    Use with: DATASET_TRANSFORM_FN="dapo_math_rlvr_tokenize rlvr_max_length_filter_v1"
    """
    # Step 1: Remap DAPO columns to rlvr format
    row["messages"] = row["source_prompt"]
    reward_model = row.get("reward_model", {})
    row["ground_truth"] = reward_model.get("ground_truth", "")
    row["dataset"] = "math"

    # Step 2: Tokenize using standard rlvr logic
...
TRANSFORM_FNS = {
    ...
    "rlvr_tokenize_v3": (rlvr_tokenize_v3, "map"),
    "rlvr_max_length_filter_v2": (rlvr_max_length_filter_v2, "filter"),
    "rubric_judge_tokenize_v2": (rubric_judge_tokenize_v2, "map"),
    "dapo_math_remap": (dapo_math_remap, "map"),
    "dapo_math_rlvr_tokenize": (dapo_math_rlvr_tokenize, "map"),
}
...
def get_dataset_v1(dc: DatasetConfig, tc: TokenizerConfig):
    # Handle case where there's 1 transform function but multiple argument dictionaries
    # Merge them into a single dictionary
    if len(dc.transform_fn) == 1 and len(dc.transform_fn_args) > 1:
        merged_args = {}
        for arg_dict in dc.transform_fn_args:
            merged_args.update(arg_dict)
        transform_fn_args = [merged_args]
    else:
        transform_fn_args = dc.transform_fn_args

    assert len(dc.transform_fn) == len(transform_fn_args), (
        f"transform_fn and transform_fn_args must have the same length: {dc.transform_fn=} != {transform_fn_args=}"
    )
    ...
    for fn_name, fn_args in zip(dc.transform_fn, transform_fn_args):
