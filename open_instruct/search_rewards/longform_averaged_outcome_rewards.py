...
def compute_longform_averaged_outcome_reward(
    ...
    extracted_context, extracted_answer, extracted_citations = extract_answer_context_citations(
        response, use_full_response_as_answer=use_full_response_as_answer
    )
    result = {
...
async def compute_longform_averaged_outcome_reward_async(
    ...
    # score format
    format_reward = compute_format_reward(response, use_full_response_as_answer=use_full_response_as_answer)
    result["format_reward"] = format_reward
    ...
    # score rubric
    scores, weights = await _score_weighted_rubric(
        extracted_answer, ground_truth, use_general_rubric=use_general_rubric, use_likert_rubric=use_likert_rubric
    )
    if len(scores) == 0:
