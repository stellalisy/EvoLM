...
from open_instruct.search_rewards.utils.rubric_chat_templates import DEFAULT_POLICY_SYSTEM_PROMPT
...
async def generate_partial_rollouts(
    model_name, messages=None, context=None, system_prompt=None, num_rollouts=1, rollout_length=8192, temperature=1
):
...
async def maybe_replace_on_policy_rollouts_with_partial_rollouts(
    ...
    for i in range(num_prompts):
        ...
        indices_to_replace = random.sample(
            range(num_samples_per_prompt_rollout),
        ...
        for model_id, local_idx in enumerate(indices_to_replace):
            ...
            if use_full_response_as_answer:
                ...
                messages = [
                    {
                        "role": "system",
                        "content": system_prompt if system_prompt is not None else DEFAULT_POLICY_SYSTEM_PROMPT,
                    },
                    {
                        "role": "user",
            ...
            task = run_litellm_async(model_name=model_name, messages=messages, max_tokens=8192, temperature=1)
            generation_tasks.append(task)
            task_metadata.append({"global_idx": global_idx, "context": context, "model_name": model_name})

    # Execute all generation tasks in parallel with concurrency limiting
    if generation_tasks:
        if verbose:
            print(
                f"🚀 Executing {len(generation_tasks)} partial rollout generations in parallel (max {max_concurrent_requests} concurrent)..."
            )
    ...
    if tokenizer is not None and masks is not None:
        ...
        for i in range(len(modified_responses)):
            if modified_responses[i] != original_decoded_responses[i]:
                ...
                original_context = (
                    original_decoded_responses[i].split("<answer>")[0]
                    if "<answer>" in original_decoded_responses[i]
                    else ""
                )
                ...
                if verbose:
                    ...
                    if original_context:
                        ...
                        original_context_mask = (
                            masks[i][:context_token_count] if context_token_count <= len(masks[i]) else masks[i]
                        )
