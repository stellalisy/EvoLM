...
def compute_multi_question_reward(
    ...
    if reward_type == "finegrained":
        ...
        return {"finegrained_scores": finegrained_scores, "log_values": log_values}
    elif reward_type == "finegrained_plus_averaged_on_reward":
        # Modify each finegrained score to incorporate the group average
        # This adds averaged_score to each individual problem's score
        modified_finegrained_scores = []
        for item in finegrained_scores:
            # Combine individual score with averaged score
            combined_score = item.score + averaged_score
            modified_finegrained_scores.append(
                FinegrainedScore(
                    score=combined_score,
                    effective_spans=item.effective_spans,
                    reward_group_id=item.reward_group_id,
                    reward_group_name=item.reward_group_name,
        ...
        print(
            f"🎀 finegrained_plus_averaged_on_reward: modified {len(modified_finegrained_scores)} scores (averaged_score={averaged_score})"
        )
        return {"finegrained_scores": modified_finegrained_scores, "log_values": log_values}
    elif reward_type == "averaged":
        print(f"🎀 averaged_score: {averaged_score}")
        return {"score": averaged_score, "log_values": log_values}
    elif reward_type == "multiplicative":
        print(f"🎀 multiplicative_score: {multiplicative_score}")
        return {"score": multiplicative_score, "log_values": log_values}
    elif reward_type == "finegrained_joining_averaged_rollout_reward":
        # Create a separate averaged reward entry alongside per-problem rewards.
        # Unlike finegrained_plus_averaged_on_reward (which adds avg to each score),
        # this creates an independent 'averaged_reward' entry with its own reward_group_id.
        # This allows the averaged reward to be normalized separately from per-problem rewards.
        # Compute average raw score across all problems
        avg_raw_score = averaged_score

        # Get response length for full span
        response_length = len(response)
        full_response_span = [(0, response_length)]

        # Create a new reward entry for the averaged reward
        # Use a different reward_group_id to distinguish it from problem-specific rewards
        avg_reward_group_id = max([s.reward_group_id for s in finegrained_scores], default=0) + 1

        avg_reward_score = FinegrainedScore(
            score=avg_raw_score,
            effective_spans=full_response_span,
            reward_group_id=avg_reward_group_id,
            reward_group_name="averaged_reward",
            query_idx=None,  # Will be set later
            response_idx=None,  # Will be set later
            advantage=None,  # Will be set during normalization
        )

        # Return original finegrained scores plus the averaged reward
        combined_scores = finegrained_scores + [avg_reward_score]
        print(f"🎀 finegrained_joining_averaged_rollout_reward: {len(combined_scores)} scores (added averaged_reward)")
        return {"finegrained_scores": combined_scores, "log_values": log_values}
    elif reward_type == "finegrained_plus_averaged_on_advantage":
        # This creates a placeholder for averaged advantage that will be filled after normalization
        # Return the original finegrained scores with a marker
        # The actual averaged advantage will be computed in fgrpo_fast.py after normalization
        print(
            f"🎀 finegrained_plus_averaged_on_advantage: {len(finegrained_scores)} scores (will add averaged_advantage after normalization)"
        )
        return {
            "finegrained_scores": finegrained_scores,
            "log_values": {**log_values, "_add_averaged_advantage": True},
        }
    else:
