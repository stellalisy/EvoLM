...
def upload_scholarqabench_data(
    use_system_prompt: bool = True, use_new_mcp_system_prompt: bool = False
) -> list[dict[str, Any]]:
    """
    ...
    """
    with open("test_configs_snippets.json") as f:
        data = json.load(f)
...
def upload_longform_sqa_train_data(
    use_system_prompt: bool = True,
    use_new_mcp_system_prompt: bool = False,
    reward_type: str = "rubrics_only",
    rubric_type: str = "no_retrieval_1k",
) -> list[dict[str, Any]]:
    data = []
    with open(f"open_instruct/search_rewards/data/rubrics_{rubric_type}.jsonl") as f:
        for line in f:
...
def upload_longform_surveyqa_validation_data(
    use_system_prompt: bool = True, use_new_mcp_system_prompt: bool = False, reward_type: str = "rubrics_only"
) -> list[dict[str, Any]]:
    ...
    if use_system_prompt and not use_new_mcp_system_prompt:
        if reward_type == "rubrics_only":
            ...
            dataset.push_to_hub(
                f"{HF_USERNAME}/rl_rag_surveyqa_validation_longform_averaged_outcome_with_system_prompt"
            )
        elif reward_type == "finegrained":
...
if __name__ == "__main__":
    ...
    data = upload_longform_sqa_train_data(
        use_system_prompt=True,
        use_new_mcp_system_prompt=True,
        reward_type="rubrics_only",
        rubric_type="searcharena_rubrics_web_augmented",
    )
