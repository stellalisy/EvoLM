...
def _prepare_asearcher_samples(asearcher_dataset) -> list[dict[str, Any]]:
    ...
    if not processed_samples:
        raise ValueError("No valid samples found in ASearcher dataset with non-empty question and aug_answer.")
...
def create_multi_question_examples_from_asearcher_base(
    ...
    print(
        f"Generating {num_examples} multi-question examples with {num_questions} questions each from ASearcher Base..."
    )
...
def load_tqa_dataset():
    ...
    try:
        ...
        tqa_file = hf_hub_download(
            repo_id="rulins/tqa_rlvr_no_prompt", filename="data/train-00000-of-00001.parquet", repo_type="dataset"
        )
...
def load_2wiki_dataset():
    ...
    try:
        ...
        wiki2_file = hf_hub_download(
            repo_id="rulins/2wiki_rlvr_no_prompt", filename="data/train-00000-of-00001.parquet", repo_type="dataset"
        )
...
def load_tqa_test_dataset():
    ...
    try:
        ...
        tqa_test_file = hf_hub_download(
            repo_id="rulins/tqa_rlvr_no_prompt", filename="data/test-00000-of-00001.parquet", repo_type="dataset"
        )
...
def load_2wiki_test_dataset():
    ...
    try:
        ...
        wiki2_test_file = hf_hub_download(
            repo_id="rulins/2wiki_rlvr_no_prompt", filename="data/test-00000-of-00001.parquet", repo_type="dataset"
        )
...
def load_seed_datasets():
    ...
    try:
        ...
        hotpot_file = hf_hub_download(
            repo_id="rulins/hotpotqa_rlvr_no_prompt", filename="data/train-00000-of-00001.parquet", repo_type="dataset"
        )
...
def create_multi_question_examples_from_2wiki_and_hotpotqa(
    hotpotqa_dataset, wiki2_dataset, num_examples: int = 1000, seed: int = 42
):
    """
...
def create_multi_question_examples_from_tqa(
    tqa_dataset, num_examples: int = 1000, seed: int = 42, num_questions: int = 2
):
    """
    ...
    if num_examples == -1:
        ...
        if len(tqa_list) < total_questions_needed:
            raise ValueError(
                f"Not enough questions in TQA dataset after filtering. Need {total_questions_needed} but only have {len(filtered_tqa_list)}. Maximum possible examples: {max_possible_examples}"
            )
...
def create_multi_question_examples_from_2wiki(
    wiki2_dataset, num_examples: int = 1000, seed: int = 42, num_questions: int = 2
):
    """
    ...
    if num_examples == -1:
        ...
        if len(wiki2_list) < total_questions_needed:
            raise ValueError(
                f"Not enough questions in 2Wiki dataset after filtering. Need {total_questions_needed} but only have {len(filtered_wiki2_list)}. Maximum possible examples: {max_possible_examples}"
            )
    ...
    print(
        f"Generating {num_examples} multi-question examples with {num_questions} questions each from 2Wiki dataset..."
    )
...
def create_multi_question_dataset_from_tqa(
    num_examples: int = 1000,
    seed: int = 42,
    num_questions: int = 2,
    push_to_hub: bool = False,
    hub_id: str = None,
    dataset_name: str = "rl_rag_toy_case_multi_dataset_single_source_tqa",
    val_size: int = 300,
):
    """
...
def create_multi_question_dataset_from_2wiki(
    num_examples: int = 1000,
    seed: int = 42,
    num_questions: int = 2,
    push_to_hub: bool = False,
    hub_id: str = None,
    dataset_name: str = "rl_rag_toy_case_multi_dataset_single_source_2wiki",
    val_size: int = 300,
):
    """
...
def create_multi_question_dataset(
    num_examples: int = 1000, seed: int = 42, push_to_hub: bool = False, hub_id: str = None
):
    """
...
def create_balanced_multi_question_dataset(
    num_examples: int = 1000,
    seed: int = 42,
    push_to_hub: bool = False,
    hub_id: str = None,
    dataset_name: str = "rl_rag_toy_case_multi_dataset_finegrained",
):
    """
...
if __name__ == "__main__":
    ...
    asearcher_base_dataset = create_multi_question_dataset_from_asearcher_base(
        num_examples=1000, seed=42, num_questions=num_questions, push_to_hub=True, hub_id=hub_id
    )
