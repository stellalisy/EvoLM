...
base_data = processed_base["train"].remove_columns(
    [col for col in processed_base["train"].column_names if col not in ["messages", "ground_truth", "dataset"]]
)
lrm_data = processed_lrm["train"].remove_columns(
    [col for col in processed_lrm["train"].column_names if col not in ["messages", "ground_truth", "dataset"]]
)
...
final_dataset = DatasetDict({"ASearcherBase35k": base_data, "ASearcherLRM35k": lrm_data})
