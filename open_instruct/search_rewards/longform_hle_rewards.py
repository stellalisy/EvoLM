...
from .utils.judge_prompts import HLE_JUDGE_SCORE_PROMPT as HLE_JUDGE_PROMPT
from .utils.judge_prompts import (
    extract_hle_score_judge_response_from_response as extract_hle_judge_response_from_response,
)
...
def hle_judge_reward(question: str, response: str, correct_answer: str, no_reasoning: bool = False) -> dict[str, Any]:
    if no_reasoning:
        judge_prompt = HLE_JUDGE_PROMPT_NO_REASONING.format(
    ...
    judge_response = run_litellm(
        model_name=os.environ.get("HLE_JUDGE_MODEL", "gpt-4.1"), system_prompt=None, user_prompt=judge_prompt
    )
    return judge_response


def compute_hle_reward(
    response: str, correct_answer: str, question: str, no_reasoning: bool = False
) -> dict[str, Any]:
    """
    Returns:
    ...
    # Step 4: Final score
    scoring_results = {"judge_score": judge_reward["reward"], "citations_score": citations_score}
