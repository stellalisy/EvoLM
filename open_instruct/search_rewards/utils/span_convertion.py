...
def find_message_spans(messages: list[dict[str, str]], formatted_text: str) -> list[dict]:
    """
...
def apply_chat_template(
    ...
    *,
    ...
    formatted_text = tokenizer.apply_chat_template(
        messages, tokenize=False, template=template, add_generation_prompt=add_generation_prompt
    )
...
def map_token_offsets_to_messages(offset_mapping: torch.Tensor, enriched_messages: list[dict]) -> list[int]:
    """
...
def message_span_aware_tokenization(
    tokenizer: PreTrainedTokenizer, messages: list[dict[str, str]], add_generation_prompt: bool = False, **kwargs
):
    formatted_text, messages = apply_chat_template(
        tokenizer, messages, template=None, add_generation_prompt=add_generation_prompt
    )
    ...
    token_message_indices = map_token_offsets_to_messages(tokenized_input["offset_mapping"], messages)
    message_token_spans = get_message_token_spans(token_message_indices)

    if kwargs.get("return_tensors") == "pt":
        token_message_indices = torch.LongTensor([token_message_indices])  # unsqueezed version
...
def locate_token_string_in_original_input(
    ...
    target_message_span_index: int | None = None,
    ...
    document_message_span = tokenized_input["message_token_spans"][target_message_span_index]
    assert all(
        [
    ...
    document_message_start_string_index = tokenized_input["offset_mapping"][document_message_span[0]][0]
...
def convert_string_spans_to_token_spans(
    tokenized_input: dict[str, Any], message_string_spans: list[tuple[int, int]], target_message_span_index: int
) -> int:
    """
    ...
    for local_string_start, local_string_end in message_string_spans:
        ...
        for token_index, (token_string_start, token_string_end) in enumerate(
            tokenized_input["offset_mapping"][message_start_token_index:message_end_token_index],
            start=message_start_token_index,
        ):
...
def decode_with_offset_mapping(
    token_ids: list[int], tokenizer: PreTrainedTokenizer, skip_special_tokens: bool = False
):
    if not hasattr(tokenizer, "clean_vocab_id_to_token_map"):
        warnings.warn("Tokenizer does not have clean_vocab_id_to_token_map attribute. We try to create one")
        token_list, token_indices = clean_tokenizer_vocab(tokenizer)
        tokenizer.clean_vocab_id_to_token_map = {idx: token for token, idx in zip(token_list, token_indices)}
...
def find_tag_spans_in_text(text: str, search_tag: str, search_offset: int = 0) -> list[tuple]:
    """
