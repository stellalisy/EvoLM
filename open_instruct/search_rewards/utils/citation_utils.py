...
def extract_claims_and_corresponding_citation_ids(
    response: str, split_non_cited_parts_by_newlines: bool = False, split_non_cited_parts_by_sentences: bool = False
) -> dict[str, list[str]]:
    """
    Example response:
...
async def score_citation_recall_async(
    question: str, claim: str, concatenated_citations: str, full_response: str
) -> float:
    if len(concatenated_citations) == 0:
