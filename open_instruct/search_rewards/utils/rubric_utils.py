...
async def _score_property_async(
    response: str,
    question: str,
    prop: str,
    system_prompt: str = None,
...
Your JSON Evaluation:"""
        score = _score_property(
            None, None, None, system_prompt=system_prompt, user_prompt=user_prompt, score_scale=10.0
        )
...
async def _score_weighted_rubric(
    response: str, ground_truth: dict[str, Any], use_general_rubric: bool = False, use_likert_rubric: bool = False
...
async def generate_instance_wise_adaptive_rubrics(question, response_list, existing_rubrics=None):
    ...
    try:
        resp = await run_litellm_async(model_name="gpt-4.1", user_prompt=prompt)
...
async def _generate_instance_wise_adaptive_rubrics(
    responses, ground_truths, num_samples_per_prompt_rollout, rubric_buffer=None
):
...
def update_ground_truths_with_adaptive_rubrics(
    ground_truths, all_adaptive_rubrics, num_samples_per_prompt_rollout, rubric_buffer=None
):
    """
    ...
    for i, (ground_truth, adaptive_rubrics) in enumerate(zip(ground_truths, expanded_adaptive_rubrics)):
        ...
        if rubric_buffer is not None and query in rubric_buffer and query not in processed_queries:
            print(
                f"Updating rubric buffer for query {query}; before update, there is {len(rubric_buffer[query]['active_rubrics'])} active rubrics and {len(rubric_buffer[query]['inactive_rubrics'])} inactive rubrics"
            )
