...
def compute_direction_agreement(
    log_values: dict[str, Any], verifiable_rewards: list[float], filter_responses_with_all_zero_rewards: bool = True
) -> dict[str, Any]:
    fine_grained_keys = ["format_reward", "num_search_turns_reward", "rubric_reward", "citation_reward"]
    ...
    if filter_responses_with_all_zero_rewards:
        ...
        if len(filtered_global_rewards) <= 1:
            print(
                f"Warning: Only {len(filtered_global_rewards)} examples left after filtering. Returning NaN correlations."
            )
...
def compute_direction_agreement_per_prompt(
    prompts: list[str], persistent_rubric_rewards: list[float], adaptive_rubric_rewards: list[float]
) -> dict[str, Any]:
    """
    ...
    if not (len(prompts) == len(persistent_rubric_rewards) == len(adaptive_rubric_rewards)):
        raise ValueError(
            f"All input lists must have the same length. Got: {len(prompts)=}, {len(persistent_rubric_rewards)=}, {len(adaptive_rubric_rewards)=}"
        )
    ...
    if spearman_correlations:
        ...
        for key in [
            ...
            "spearman_min",
            "spearman_max",
            "fraction_positive_correlation",
            "fraction_strong_agreement",
