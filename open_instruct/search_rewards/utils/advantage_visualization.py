...
def convert_string_span_to_token_span(
    effective_spans: list[tuple[int, int]], decoded_resp: str, token_resp: list[int], tokenizer: PreTrainedTokenizer
) -> list[int]:
    """
...
def compute_token_advantages(
    response: str, finegrained_rewards: Any, tokenizer: PreTrainedTokenizer
    ...
    for score_obj in finegrained_rewards.finegrained_scores:
        ...
        span_mask = convert_string_span_to_token_span(score_obj.effective_spans, decoded_resp, token_ids, tokenizer)
...
def visualize_advantage_application(
    response: str,
    ...
    token_advantages, token_ids, decoded_resp = compute_token_advantages(response, finegrained_rewards, tokenizer)
    ...
    print_colored(
        f"   📊 Tokens: 🔴{positive_tokens} positive, 🔵{negative_tokens} negative, ⚪{zero_tokens} neutral",
        Colors.CYAN,
    )
    ...
    if show_token_details:
        print_colored("   🔤 Token Details:", Colors.YELLOW)
