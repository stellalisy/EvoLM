...
def run_chatopenai(
    model_name: str, system_prompt: str | None, user_prompt: str, json_mode: bool = False, **chat_kwargs
) -> str:
    ...
    resp = litellm.completion(model=model_name, messages=msgs, **chat_kwargs)
...
def run_azure_openai(
    ...
    **chat_kwargs,
    ...
    response = client.chat.completions.create(messages=msgs, model=deployment, **chat_kwargs)
...
def run_litellm(model_name: str, user_prompt: str, system_prompt: str | None = None, **chat_kwargs) -> str:
    ...
    chat_kwargs["frequency_penalty"] = chat_kwargs.get("frequency_penalty", 0.0)
    chat_kwargs["presence_penalty"] = chat_kwargs.get("presence_penalty", 0.0)
    chat_kwargs["num_retries"] = chat_kwargs.get("num_retries", int(os.environ.get("LITELLM_NUM_RETRIES", "60")))
    # Only set fallback if not already specified and not using hosted_vllm (which shouldn't fallback to GPT)
    if "fallbacks" not in chat_kwargs:
        # If using hosted_vllm, don't set GPT fallback (server should be reliable)
        if not model_name.startswith("hosted_vllm/"):
            chat_kwargs["fallbacks"] = ["gpt-4.1-mini"]
