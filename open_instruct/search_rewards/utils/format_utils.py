...
def extract_answer_context_citations(
    response: str, use_full_response_as_answer: bool = False
    ...
    if not match:
        ...
        LOGGER.warning(
            f"No <answer></answer> tags found in response with use_full_response_as_answer set to {use_full_response_as_answer}"
        )
...
def extract_search_tool_calls(context: str, mcp_parser_name: str | None = None) -> list[str]:
...
def compute_format_reward(
    response: str, mcp_parser_name: str | None = None, use_full_response_as_answer: bool = False
) -> float:
