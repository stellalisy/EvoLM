...
@dataclass
class RubricJudgeVerifierConfig(VerifierConfig):
    rubric_judge_model: str = "gpt-4o-mini"
    """Model to use for generating rubrics and judging answers"""
    ...
    """Number of logprobs to return (default: 1 to match normal training path)"""
    rubric_correctness_focused: bool = False
    """Use correctness-focused rubric generation prompt"""
    rubric_reward_score_separation: bool = False
    """Add score separation bonus to rubric reward"""
    rubric_reward_score_separation_weight: float = 0.3
    """Weight of score separation bonus (final_reward = (1-w)*pairwise + w*margin)"""
...
class VerificationResult:
    score: float
    cost: float = 0.0
    log_values: dict[str, float] | None = None
...
class RLRAGLongFormRubricsOnlyVerifier(VerifierFunction):
    ...
    def __init__(self, verifier_config: RubricVerifierConfig | None = None) -> None:
    ...
    def __call__(
        ...
        from open_instruct.search_rewards.longform_rubric_rewards import compute_rubric_reward

        test_case = json.loads(label)
        use_general_rubric = self.verifier_config.use_general_rubric if self.verifier_config else False
        evaluate_closed_book_answer = (
            self.verifier_config.evaluate_closed_book_answer if self.verifier_config else False
        )
        ...
        return VerificationResult(score=score, log_values=result.get("log_values"))


class RLRAGLongFormGeneralRubricVerifier(VerifierFunction):
    """
    Verifier that computes the RL-RAG (long form) score between the prediction and the label.
    Uses weighted rubric reward with citation and format rewards.
    """
    ...
    def __call__(
        ...
        from open_instruct.search_rewards.longform_rubric_rewards import (
            compute_weighted_rubric_reward_with_citation_and_format_reward,
        )

        test_case = json.loads(label)
        use_general_rubric = self.verifier_config.use_general_rubric if self.verifier_config else False
        no_citation_reward = self.verifier_config.no_citation_reward if self.verifier_config else False
        use_likert_rubric = self.verifier_config.use_likert_rubric if self.verifier_config else False

        result = compute_weighted_rubric_reward_with_citation_and_format_reward(
            prediction,
            test_case,
            use_general_rubric=use_general_rubric,
            no_citation_reward=no_citation_reward,
...
class RLRAGLongFormAveragedOutcomeVerifier(VerifierFunction):
    """
    Verifier that computes the RL-RAG (long form) averaged outcome score between the prediction and the label.
    """
    ...
    def __call__(
        ...
        from open_instruct.search_rewards.longform_averaged_outcome_rewards import (
            compute_longform_averaged_outcome_reward,
        )
        ...
        no_citation_reward = self.verifier_config.no_citation_reward if self.verifier_config else False
        use_likert_rubric = self.verifier_config.use_likert_rubric if self.verifier_config else False
        use_full_response_as_answer = (
            self.verifier_config.use_full_response_as_answer if self.verifier_config else False
        )
...
class RubricJudgeVerifier(VerifierFunction):
    """
    Verifier that uses rubric generation and LLM-as-a-judge to evaluate answers.

    The model generates a rubric from the question, then an LLM-as-a-judge evaluates
    both accepted and rejected answers. Reward is produced if accepted answer is correct.
    """

    def __init__(
        self,
        verifier_config: RubricJudgeVerifierConfig | None = None,
        rubric_judge_generate_text_actor: Any | None = None,
    ) -> None:
        super().__init__("rubric_judge", verifier_config=verifier_config, weight=1.0)
    ...
    def get_config_class(cls) -> type:
        ...
        Return the configuration class for this verifier.
        """
        return RubricJudgeVerifierConfig

    async def async_call(
        self, tokenized_prediction: list[int], prediction: str, label: str, query: str | None = None
    ) -> VerificationResult:
        """
        Asynchronously compute rubric judge reward.

        Args:
            tokenized_prediction: Unused tokenized representation
            prediction: The generated rubric from the model
            label: JSON string containing question, accepted_answer, rejected_answer
            query: Unused original query
        ...
        ground_truth = json.loads(label)
        question = ground_truth["question"]
        accepted_answer = ground_truth["accepted_answer"]
        rejected_answer = ground_truth["rejected_answer"]

        # Use the model's generated rubric
        generated_rubric = prediction.strip()

        # Get rubric judge model and sampling params from config if available
        judge_model = None
        rubric_model = None
        sampling_params = None
        if self.verifier_config and isinstance(self.verifier_config, RubricJudgeVerifierConfig):
            judge_model = self.verifier_config.rubric_judge_model
            rubric_model = self.verifier_config.rubric_judge_model
            # Create sampling params from config
            import vllm

            sampling_params = vllm.SamplingParams(
                temperature=self.verifier_config.rubric_judge_temperature,
                top_p=0.95,
                max_tokens=self.verifier_config.rubric_judge_max_tokens,
                stop=self.verifier_config.rubric_judge_stop,
                logprobs=self.verifier_config.rubric_judge_logprobs,
            )

        score_separation = False
        score_separation_weight = 0.3
        if self.verifier_config and isinstance(self.verifier_config, RubricJudgeVerifierConfig):
            score_separation = self.verifier_config.rubric_reward_score_separation
            score_separation_weight = self.verifier_config.rubric_reward_score_separation_weight
        ...
        result = await compute_rubric_judge_reward_async(
            ...
            score_separation_weight=score_separation_weight,
        )
        ...
        return VerificationResult(score=score, log_values=log_values, reasoning=result.get("accepted_reasoning", ""))

    def __call__(
        self, tokenized_prediction: list[int], prediction: str, label: str, query: str | None = None
    ) -> VerificationResult:
        """
        Synchronously compute rubric judge reward.
        """
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
...
class RewardModelVerifierConfig(VerifierConfig):
    ...
    """HuggingFace model name for scalar reward model"""
...
class RewardModelVerifier(VerifierFunction):
    """
    Verifier that scores (query, response) pairs using a pool of scalar reward
    model replicas (e.g. Bradley-Terry trained AutoModelForSequenceClassification).
    Requests are round-robined across replicas to avoid stalling the training pipeline.
    """

    def __init__(self, verifier_config: RewardModelVerifierConfig, reward_model_actors: list | None = None) -> None:
        super().__init__("reward_model", verifier_config=verifier_config, weight=1.0)
        self.reward_model_actors = reward_model_actors or []
        self._next_actor = 0
    ...
    def _pick_actor(self):
        """Round-robin across the actor pool."""
        if not self.reward_model_actors:
            raise RuntimeError("RewardModelVerifier requires at least one reward_model_actor")
        actor = self.reward_model_actors[self._next_actor % len(self.reward_model_actors)]
        self._next_actor += 1
        return actor
    ...
    def __call__(
        ...
        raise RuntimeError("RewardModelVerifier only supports async_call (requires Ray actor pool)")
    ...
    async def async_call(
        ...
        score = await actor.score.remote(query, prediction)
        return VerificationResult(score=score)


def _create_reward_model_actors(config: RewardModelVerifierConfig) -> list:
    """Create a pool of Ray actors (one per GPU) that each host the scalar reward model."""
    import ray

    @ray.remote(num_gpus=1)
    class RewardModelActor:
        def __init__(self, model_name: str, gpu_memory_utilization: float):
            import torch
            from transformers import AutoModelForSequenceClassification, AutoTokenizer

            self.tokenizer = AutoTokenizer.from_pretrained(model_name)
            self.model = AutoModelForSequenceClassification.from_pretrained(
                model_name, torch_dtype=torch.bfloat16, device_map="auto", num_labels=1
            )
            self.model.eval()
        ...
        def score(self, query: str, response: str) -> float:
            ...
            with torch.no_grad():
                logit = self.model(**inputs).logits[0][0].item()
            return torch.sigmoid(torch.tensor(logit)).item()

    n_replicas = config.reward_model_num_gpus
    actors = []
    for i in range(n_replicas):
        actor = RewardModelActor.options(name=f"reward_model_actor_{i}", max_concurrency=64).remote(
            config.reward_model_name, config.reward_model_gpu_memory_utilization
        )
        actors.append(actor)
    logger.info("Created %d reward model replicas on %d GPUs", n_replicas, n_replicas)
    return actors


def build_all_verifiers(args, rubric_judge_generate_text_actor=None) -> dict[str, VerifierFunction]:
    """
    ...
    for subclass in VerifierFunction.__subclasses__():
        if subclass == LMJudgeVerifier:
            continue
        if subclass == RewardModelVerifier:
            continue

        verifier_config = subclass.get_config_class().from_args(args)
        if subclass == RubricJudgeVerifier:
            instance = subclass(verifier_config, rubric_judge_generate_text_actor)
        else:
    ...
    # If a scalar reward model is configured, override ALL verifiers so every
    # dataset routes through the reward model regardless of its verifier tag.
    reward_model_name = getattr(args, "reward_model_name", None)
    if reward_model_name:
        rm_config = RewardModelVerifierConfig.from_args(args)
        rm_actors = _create_reward_model_actors(rm_config)
        rm_verifier = RewardModelVerifier(rm_config, reward_model_actors=rm_actors)
        for key in list(verifiers.keys()):
            verifiers[key] = rm_verifier
        verifiers["reward_model"] = rm_verifier
        logger.info("Reward model '%s' overrides all verifiers (%d replicas)", reward_model_name, len(rm_actors))
