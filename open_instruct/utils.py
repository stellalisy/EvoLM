...
def sanitize_wandb_tags(tags: Iterable[str]) -> list[str]:
    """Normalize W&B tags to satisfy service constraints and preserve order."""
    sanitized = []
    seen = set()
    for tag in tags:
        if tag is None:
...
def get_wandb_tags() -> list[str]:
    ...
    return sanitize_wandb_tags(tags)
...
def maybe_use_ai2_wandb_entity() -> str | None:
    ...
    try:
        ...
        # If wandb API is unavailable (e.g., network issues, auth issues), gracefully return None
        print(f"Warning: Unable to check wandb team membership: {e}")
        return None
...
def maybe_use_ai2_hf_entity() -> str | None:
    ...
    try:
        ...
        # If HuggingFace API is unavailable (e.g., network issues), gracefully return None
        print(f"Warning: Unable to check HuggingFace organization membership: {e}")
        return None
...
GPU_SPECS = {
    ...
    "h200": {"flops": 990e12, "memory_size": 141e9, "memory_bandwidth": 4.8e12},  # 4.8 TB/s HBM3e (141GB variant)
