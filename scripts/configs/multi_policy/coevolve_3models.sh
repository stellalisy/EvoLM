#!/bin/bash
# Multi-policy CO-EVOLUTION configuration: 3 policies (1 main + 2 extra)
# Main: Qwen3-8B (standard resources), Extra: Llama-3-8B + Mistral-7B
# ALL three policies receive gradient updates and alternate with rubric training.

# Extra co-evolving policies (main policy is HF_CHECKPOINT, listed separately)
# Note: Use Llama-3.1 (128K context) instead of Llama-3 (8K context) to fit response_length=16384
MULTI_POLICY_COEVOLVE_MODELS="meta-llama/Llama-3.1-8B-Instruct,mistralai/Mistral-7B-Instruct-v0.3"
