...
# Using OLMo-3-7B-Think as both policy and rubric generator
# Mirrors qwen3_8b_8nodes_singlemodel.sh but with OLMo-3-7B-Think

# Assert that single model mode is enabled when using this config
if [ "${SINGLE_MODEL_MODE:-false}" != "true" ]; then
    echo "ERROR: This config file (olmo3_7b_8nodes_singlemodel.sh) requires SINGLE_MODEL_MODE=true"
    echo "Please include 'single_model' config before this config in your launch command:"
    echo "  ./scripts/launch.sh alternating_training single_model ... olmo3_7b_8nodes_singlemodel"
    exit 1
fi

HF_CHECKPOINT=${HF_CHECKPOINT:-allenai/Olmo-3-7B-Think}

# Set experiment name
if [ -z "$EXP_NAME_BASE" ]; then
    EXP_NAME_BASE="olmo3_7b_8nodes_alt_n\${NUM_UNIQUE_PROMPTS_ROLLOUT}"
...
RUBRIC_MODEL="${RUBRIC_MODEL:-allenai/Olmo-3-7B-Think}"
...
echo "[olmo3_7b_8nodes_singlemodel] Configuration:"
