#!/bin/bash
# Multi-judge configuration: 2 judges
# Uses Qwen3-1.7B and Llama-3-8B as judges
...
# Multi-judge replaces the single-judge engines, but the shared rubric_judge
# engine settings are still used. Resolve them from the per-model rubric_judge
# configs unless they were explicitly overridden.
