...
if [ "${SINGLE_MODEL_MODE:-false}" != "true" ]; then
    echo "ERROR: This config file (qwen3_8b_8nodes_alt_single_model_vllm_judge.sh) requires SINGLE_MODEL_MODE=true"
    echo "Please include 'single_model' config before this config in your launch command:"
    echo "  ./scripts/launch.sh alternating_training single_model ... qwen3_8b_8nodes_alt_single_model_vllm_judge"
    exit 1
fi
...
if [ -z "$EXP_NAME_BASE" ]; then
    EXP_NAME_BASE="qwen3_8b_8nodes_alt_nup\${NUM_UNIQUE_PROMPTS_ROLLOUT}"
...
echo "[qwen3_8b_8nodes_alt_single_model_vllm_judge] Configuration:"
...
# Usage:
#   ./scripts/launch.sh alternating_training single_model dpo_model_ladder rubric_judge_server_vllm qwen3_8b_8nodes_alt_single_model_vllm_judge
#
# Or with tensor parallelism:
#   ./scripts/launch.sh alternating_training single_model dpo_model_ladder rubric_judge_server_vllm qwen3_8b_8nodes_alt_single_model_vllm_judge DEEPSPEED_TENSOR_PARALLEL_SIZE=2
#
# This will create:
#   - Alternating training setup in SINGLE MODEL MODE (shared vLLM engines)
#   - Combined vLLM engines: VLLM_NUM_ENGINES + RUBRIC_JUDGE_NUM_ENGINES = 32 total
#   - Total GPUs needed: NUM_LEARNERS_PER_NODE * num_nodes * DEEPSPEED_TENSOR_PARALLEL_SIZE
#   - vLLM engines: 32 (shared between rubric and policy actors)